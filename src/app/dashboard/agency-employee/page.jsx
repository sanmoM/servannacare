  ...
  return <div className=" mx-auto bg-gray-50 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
          <Button onClick={handleAddEmployeeClick} className="bg-primary shadow-sm w-full sm:w-auto cursor-pointer" isActionLoading={isActionLoading}>
          ...
          <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <DashboardAddEmployees onSuccess={() => {
            refetch();
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {employees.length > 0 ? employees.map(emp => <tr key={emp.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-lg bg-purple-100 flex-shrink-0 border overflow-hidden">
                          <img src={`${process.env.NEXT_PUBLIC_IMAGE_BASE_URL}${emp?.profilePhoto}`} alt={emp.name} className="h-full w-full object-cover" onError={e => {
                      e.target.src = "https://ui-avatars.com/api/?name=" + emp.name;
                    }} />
                        </div>
                        <div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end items-center gap-1">
                        {emp.is_paid ? emp.paid_ends_at ? <div className="mr-2 px-3 py-1 rounded-md bg-emerald-50 border border-emerald-200">
                              <p className="text-[11px] font-semibold text-emerald-700">
                                Paid Until
                              </p>

                              <p className="text-xs text-emerald-600">
                                {new Date(emp.paid_ends_at).toLocaleDateString("en-GB").replaceAll("/", "-")}
                              </p>
                            </div> : <div className="mr-2 px-3 py-1 rounded-md bg-blue-50 border border-blue-200">
                              <p className="text-xs font-semibold text-primary-700">
                                Free Employee
                              </p>
                            </div> : <Button variant="default" size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white mr-2 h-8 px-3 cursor-pointer" onClick={() => {
                    ...
                    setPaymentModalOpen(true);
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="text-primary-600 hover:bg-blue-50 cursor-pointer" isActionLoading={isActionLoading}>
                              <Eye size={18} />
                            </Button>
                          </DialogTrigger>
                        ...
                        <Dialog open={scheduleViewId === emp.id} onOpenChange={open => !open && setScheduleViewId(null)}>
                          <DialogContent className="sm:max-w-md">
                            <DialogHeader>
                              <DialogTitle>Selected Schedule</DialogTitle>
                            </DialogHeader>

                            <SelectableCalendar mode="multiple" selectedDates={emp.schedule?.[0]?.date || []} onChange={() => {}} />
                          </DialogContent>
                        </Dialog>

                        <Dialog open={editModalId === emp.id} onOpenChange={open => !open && setEditModalId(null)}>
                          <DialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="text-emerald-600 hover:bg-emerald-50 cursor-pointer" onClick={() => setEditModalId(emp.id)} isActionLoading={isActionLoading}>
                              <Edit size={18} />
                            </Button>
                          </DialogTrigger>
                          ...
                          <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
                            ...
                            <AgencyEmployee initialData={emp} isUpdate={true} onSuccess={() => {
                        refetch();
                        setEditModalId(null);
                      }} />
                          </DialogContent>
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="text-red-500 hover:bg-red-50 cursor-pointer" isActionLoading={isActionLoading}>
                              <Trash size={18} />
                            </Button>
                          </DialogTrigger>
                          <DialogContent className="sm:max-w-md">
                            ...
                            <DialogFooter className="gap-2 sm:gap-0">
                              <DialogClose asChild>
                                <Button className="cursor-pointer" variant="outline" isActionLoading={isActionLoading}>
                                  Cancel
                                </Button>
                              </DialogClose>
                              <Button className="cursor-pointer" variant="destructive" onClick={() => handleDelete(emp.id)} isActionLoading={isActionLoading}>
                                Confirm Delete
                              </Button>
                            </DialogFooter>
                          </DialogContent>
      ...
      <PaymentModal open={paymentModalOpen} onOpenChange={setPaymentModalOpen} employeeIds={paymentEmployeeIds} onSuccess={() => refetch()} />
    </div>;
};
const InfoTile = ({
  label,
