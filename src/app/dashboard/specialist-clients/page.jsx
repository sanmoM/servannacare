...
const Page = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      const sortedData = [...rawData].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  ...
  const filteredClients = activeFilter === "all" ? clients : clients.filter(c => c.booking_status === activeFilter);
  ...
  return <div className="w-full sm:px-6 lg:px-8 py-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              return <tr key={row.id} className="hover:bg-green-50/10 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-full bg-green-50 text-primary flex items-center justify-center font-bold text-sm border border-green-100">
                            ...
                            {row?.patient_name ? row?.patient_name?.charAt(0) : row?.user?.name?.charAt(0)}
                          </div>
                          <div>
                            <div className="font-semibold text-gray-900">
                              {row?.patient_name ? row?.patient_name : row?.user?.name}
                            </div>
                            <div className="text-xs text-gray-500">
                              {row?.patient_age && row.patient_age}
                              {row.patient_gender && `yrs • ${row.patient_gender}`}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm">
                        ...
                        <div className="text-xs text-gray-400">
                          {row?.relationship_to_booking_person ? `Guardian • ${row?.relationship_to_booking_person}` : "N/A"}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
        ...
        {filteredClients.length > itemsPerPage && <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
