...
const Nurse = ({
  ...
  const handleNext = async dataForStep => {
    if (step === 1) setFormData(prev => ({
      ...prev,
      basicInfo: dataForStep
    }));
    if (step === 2) setFormData(prev => ({
      ...prev,
      education: dataForStep
    }));
    if (step === 3) setFormData(prev => ({
      ...prev,
      experience: dataForStep
    }));
    if (step === 4) setFormData(prev => ({
      ...prev,
      skillsServices: dataForStep
    }));
    if (step === 5) setFormData(prev => ({
      ...prev,
      documents: dataForStep
    }));
    if (step < totalSteps) {
      setStep(step + 1);
    } else {
      ...
      <div className={`w-full ${!started ? "my-0" : "my-12"} max-w-[700px] bg-white`}>
        {!started ? <SignUpStart onSuccess={handleSignupSuccess} /> : <>
            ...
            <div className="space-y-8 mt-6">
              ...
              {step === 2 && <Education defaultValues={formData.education} onNext={handleNext} onBack={handleBack} />}
              {step === 3 && <Exprience defaultValues={formData.experience} onNext={handleNext} onBack={handleBack} />}
              {step === 4 && <SkillServices defaultValues={formData.skillsServices} onNext={handleNext} onBack={handleBack} skills={skills} />}
              {step === 5 && <DocumentUploads defaultValues={formData.documents} onNext={handleNext} onBack={handleBack} />}
              {/* {step === 6 && (
                <ContactAgreement
              ...
              {step === 6 && <Review data={formData} onNext={handleNext} onBack={handleBack} />}
            </div>
          </>}
      </div>
    </div>;
