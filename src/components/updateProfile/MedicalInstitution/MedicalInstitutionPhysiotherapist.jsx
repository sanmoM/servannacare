...
const MedicalInstitutionPhysiotherapist = ({
  ...
  const preferredInterventions = ["Pediatric", "Orthopedic", "Rehab", "Sports", "Stroke"];
  ...
  useEffect(() => {
    if (initialData && isUpdate) {
      ...
      setExistingFiles({
        eduCertificate: initialData?.eduCertificate !== "null" ? initialData?.eduCertificate : null,
  ...
  const buildPayload = () => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (["canDrive", "hospitalBasedCare", "homeBasedCare", "isRegisterPCK"].includes(key)) {
  ...
  const handleSubmit = async e => {
    ...
    if (data.isRegisterPCK) {
      if (!data.registrationNumber) return toast.error("Registration number is required");
  ...
  return <div>
      <form className="relative pb-16" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col pb-6 md:flex-row md:gap-4 gap-6">
          <Input placeholder="Name" name="name" label="Full Name (as per ID)" value={data.name} onChange={handleChange} />
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-4">
          <div className="flex-1">
            ...
            <div className="h-5 mt-1">
              {data?.number_two && !isValidPhoneNumber(data?.number_two) && <p className="text-red-500 text-sm">
        ...
        <div className="my-6">
          ...
          education: val
            ...
            <div className="flex item-center gap-2">
              <RadioGroupItem className="cursor-pointer" value="Diploma In Physiotherapy" id="edu1" />
            ...
            <div className="flex item-center gap-2">
              <RadioGroupItem className="cursor-pointer" value="Degree In Physiotherapy" id="edu2" />
        ...
        <FileUpload title="Education Certificate (Compulsory)" accept="application/pdf,image/*" icon={<FileText size={32} />} file={data.eduCertificate || existingFiles.eduCertificate} existingFile={existingFiles.eduCertificate} onFileSelect={file => setData(p => ({
        ...p,
        eduCertificate: file
      }))} />
        ...
        <div className="py-4 mt-4">
          ...
          <RadioGroup className="flex gap-4 mt-2" value={data.isRegisterPCK === null ? "" : String(data.isRegisterPCK)} onValueChange={val => {
          ...
          {data.isRegisterPCK && <div className="mt-4">
              <Input label={"Registration Number"} placeholder="Registration Number" type="text" value={data.registrationNumber || ""} onChange={e => setData(prev => ({
            ...prev,
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap flex-col gap-2 mt-2">
            {preferredInterventions.map((pref, indx) => <div key={indx} className="flex items-center gap-2">
                <Checkbox className="cursor-pointer" id={pref} checked={data.preferred.includes(pref)} onCheckedChange={() => toggleArray("preferred", pref)} />
        ...
        <div>
          ...
          <textarea value={data.bio} name="bio" placeholder="Write a brief bio about yourself and the services you offer.." className="border text-sm mt-2 p-3 w-full rounded-md outline-primary" rows={6} onChange={handleChange} />
        </div>

        {/* Schedule */}
        {initialData && <div className="mt-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 mt-4 sm:grid-cols-2 gap-4">
            {documents.map((item, indx) => <div key={indx} className="border rounded-xl p-4">
                <FileUpload title={item.title} accept={item.accept} icon={item.icon} optional={item.optional || false} file={data.documents[item.id]} existingFile={existingFiles[item.id]} onFileSelect={file => handleFileSelect("documents", item.id, file)} />
              </div>)}
          </div>
        </div>
