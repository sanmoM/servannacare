...
const SpecialNeedCaregiversUpdate = ({
  data = {}
}) => {
  ...
  return <div>
      <form onSubmit={handleUpdate} className="space-y-6 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:gap-6 sm:gap-4">
          ...
          <div className="flex-1">
            <div className="space-y-2">
              ...
              <PhoneInputWithCountrySelect international defaultCountry={country} value={formData?.basicInfo?.phone} onChange={value => handleChange("basicInfo", "phone", value || "")} onCountryChange={countryCode => {
              setCountry(countryCode);
            }} className="phone-input-custom w-full border rounded-md px-3 py-2 bg-gray-100" />

              {formData.basicInfo.phone && !isValidPhoneNumber(formData.basicInfo.phone) && <p className="text-red-500 text-sm">Invalid phone number</p>}
            </div>
          </div>
        </div>
        ...
        {/* File Upload */}
        <div>
          <FileUpload title="Education Certificate (Compulsory)" accept="application/pdf,image/*" icon={<FileText size={32} />} optional="" file={formData?.documents?.educationCertificate} onFileSelect={file => handleFileSelect("documents", "educationCertificate", file)} />
        </div>
