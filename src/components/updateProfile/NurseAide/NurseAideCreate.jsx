...
const NurseAideCreate = ({
  ...
  const [formData, setFormData] = useState({
    basicInfo: {
      ...
      canDrive: data?.canDrive === 1 || data?.canDrive === "1" || data?.canDrive === true ? true : data?.canDrive === 0 || data?.canDrive === "0" || data?.canDrive === false ? false : null
    },
    education: {
    ...
    experience: {
      hospitalBasedCare: data.hospitalBasedCare === 1 || data.hospitalBasedCare === "1" ? true : data.hospitalBasedCare === 0 || data.hospitalBasedCare === "0" ? false : null,
      ...
      homeBasedCare: data.homeBasedCare === 1 || data.homeBasedCare === "1" ? true : data.homeBasedCare === 0 || data.homeBasedCare === "0" ? false : null,
  ...
  const skills = ["Basic Patient Care (bathing, dressing, feeding, and assisting with mobility)", "Vital Signs Monitoring (checking blood pressure, blood sugar, pulse, temperature, etc.)", "Compassion & Strong Communication Skills", "Special needs caregiver (e.g., autistic, deaf, blind)", "Elderly caregiving"];
  ...
  const handleUpdate = async e => {
    ...
    if (basicInfo.canDrive === null || basicInfo.canDrive === "") return toast.error("Please select driving option");
      ...
      <form onSubmit={handleUpdate} className="space-y-6 relative">
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-4">
          <div className="flex-1">
            <Input label="Full Name (as per ID)" name="name" placeholder="Enter your name" value={formData.basicInfo?.name} onChange={e => handleChange("basicInfo", "name", e.target.value)} />
          </div>
          <div className="flex-1">
            <Input label="Your Location" name="location" placeholder="Type your location.." value={formData.basicInfo?.location} onChange={e => handleChange("basicInfo", "location", e.target.value)} />
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:gap-4 gap-6 ">
          <div className="flex-1">
            <Input type="number" label="Age" name="age" placeholder="Your age" maxLength={2} value={formData.basicInfo?.age} onChange={e => {
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 mt-2">
              {languages.map(lan => <div key={lan.id} className="flex items-center gap-2">
                  <Checkbox className={"cursor-pointer"} id={lan.value} checked={formData.basicInfo?.languages.includes(lan.value)} onCheckedChange={() => toggleArrayItem("basicInfo", "languages", lan.value)} />
        ...
        <div>
          ...
          <RadioGroup className="flex gap-4" value={formData.basicInfo.canDrive === null ? "" : formData.basicInfo.canDrive ? "true" : "false"} onValueChange={value => handleChange("basicInfo", "canDrive", value === "true")}>
            <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <RadioGroup className="flex gap-x-4 flex-wrap" value={formData.experience?.hospitalBasedCare == null ? "" : formData.experience.hospitalBasedCare ? "Yes" : "No"} onValueChange={value => handleChange("experience", "hospitalBasedCare", value === "Yes")}>
            <div className="flex items-center gap-2">
              <RadioGroupItem className="cursor-pointer" value="Yes" id="hos1" />
        ...
        <div>
          ...
          <div className="flex flex-wrap flex-col gap-2 ">
            {preferred.map((lan, indx) => <div key={indx} className="flex items-center gap-2">
                <Checkbox className="cursor-pointer" id={lan.title} checked={formData.experience.preferred.includes(lan.title)} onCheckedChange={() => toggleArrayItem("experience", "preferred", lan.title)} />
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {skills?.map((area, idx) => <div key={idx} className="flex gap-2">
                  <Checkbox className="cursor-pointer" id={area} checked={formData?.skillsServices?.skills.includes(area)} onCheckedChange={() => toggleArrayItem("skillsServices", "skills", area)} />
