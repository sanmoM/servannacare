...
const NurseUpdate = ({
  ...
  const [formData, setFormData] = useState({
    ...
    education: {
      ...
      isNursingInKenya: data?.nurse?.isNursingInKenya === undefined ? null : Boolean(data?.nurse?.isNursingInKenya)
    },
    experience: {
  ...
  const togglepreferred = pref => {
    setFormData(prev => {
      ...
      return {
        ...
        experience: {
          ...prev.experience,
          preferred: alreadySelected ? prev.experience.preferred.filter(l => l !== pref) : [...prev.experience.preferred, pref]
        }
      };
    });
  };
  ...
  return <div>
      <form onSubmit={handleUpdate} className="space-y-6 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:gap-6 sm:gap-4">
          <div className="flex-1">
            ...
            {/* <Input
             ...
             /> */}
            <PhoneInputWithCountrySelect className="w-full border rounded-md px-3 py-2" disabled international defaultCountry={country} value={formData.basicInfo.phone} onChange={value => handleChange("basicInfo", "phone", value || "")} />
          </div>
          <div className="flex-1">
            <div>
              ...
              <div className="w-full mt-2">
                <PhoneInputWithCountrySelect className="w-full border rounded-md px-3 py-2" international defaultCountry={country} value={formData.basicInfo.number_two} onChange={value => handleChange("basicInfo", "number_two", value || "")} />
              </div>

              {formData.basicInfo.number_two && !isValidPhoneNumber(formData.basicInfo.number_two)}
            </div>
          </div>
        </div>
        ...
        <div className="border rounded-xl p-4">
          <FileUpload title="Education Certificate (Compulsory)" accept="application/pdf,image/*" icon={<FileText size={32} />} file={isEduImage ? eduFile : null} onFileSelect={file => handleFileSelect("education", "educationCertificate", file)} />

          {eduFile && !isEduImage && <FilePreview file={eduFile} alt="Education Certificate" />}
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap flex-col gap-2 ">
            {preferred.map((lan, indx) => <div key={indx} className="flex items-center gap-2">
                <Checkbox id={lan.title} checked={formData.experience?.preferred?.includes(lan.title) || false} onCheckedChange={() => togglepreferred(lan.title)} />
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {skills.map((area, idx) => <div key={idx} className="flex gap-2">
                  <Checkbox id={area} checked={formData.skillsServices.skills.includes(area)} onCheckedChange={() => toggleArrayItem("skillsServices", "skills", area)} />
        ...
        <div className="grid grid-cols-1 mt-4 sm:grid-cols-2 gap-4">
          ...
          return <div key={indx} className="border rounded-xl p-4">
                <FileUpload title={item.title} accept={item.accept} icon={item.icon} optional={item.optional || false} file={isImage ? file : null} onFileSelect={file => handleFileSelect("documents", item.id, file)} />

                {file && !isImage && <FilePreview file={file} alt={item?.title} />}
              </div>;
        })}
        </div>
