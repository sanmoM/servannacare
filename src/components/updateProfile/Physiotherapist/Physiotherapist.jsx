...
const Physiotherapist = ({
  ...
  const handleUpdate = async e => {
    ...
    if (formData.basicInfo.phone && !isValidPhoneNumber(formData.basicInfo.phone)) {
      ...
      <form onSubmit={handleUpdate} className="space-y-6 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:gap-6 sm:gap-4">
          ...
          <div className="flex-1">
            ...
            <PhoneInputWithCountrySelect international disabled defaultCountry="KE" value={formData.basicInfo.number} className="w-full border rounded-md px-3 py-2 bg-gray-100" />
          </div>
          ...
          <div className="flex-1">
            ...
            <PhoneInputWithCountrySelect international defaultCountry="KE" value={formData.basicInfo.phone} onChange={value => handleChange("basicInfo", "phone", value || "")} className="w-full border rounded-md px-3 py-2" />
          </div>
        </div>
        ...
        <FileUpload title="Education Certificate (Compulsory)" accept="application/pdf,image/*" icon={<FileText size={32} />} file={eduIsImage ? eduFile : null} onFileSelect={file => handleFileSelect("documents", "eduCertificate", file)} />
        ...
        {formData.education.isRegisterPCK && <div className="mt-6">
            ...
            <div className="mt-6">
              <FileUpload title="Practising License" accept="application/pdf,image/*" icon={<FileText size={32} />} file={licenseIsImage ? licenseFile : null} onFileSelect={file => handleFileSelect("documents", "practiceLicense", file)} />

              {licenseFile && !licenseIsImage && <FilePreview file={licenseFile} alt="Practising License" />}
            </div>
          </div>}
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <textarea value={formData?.basicInfo?.bio} name="bio" placeholder="Write a brief bio about yourself and the services you offer.." className="border text-sm mt-2 p-3 w-full rounded-md outline-primary" rows={6} onChange={handleChange} />
          </div>
        </div>
