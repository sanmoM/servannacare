...
const validateEmployee = data => {
  ...
  if (data.handlePets === null || data.handlePets === undefined) errors.push("Please select if you can handle pets");
  ...
  if (!data.goodConductCertificate) errors.push("Good conduct certificate required");
...
const res = await postApi("/agency-employee", empFd, {
  ...
  return <div className="space-y-8 pb-10">
      {employees.map((num, index) => <div key={index} className="relative">
          <EmployeDetails employeeNumber={index + 1} onDataChange={data => handleEmployeeChange(index, data)} defaultValues={allEmployees[index] || {}} />
          {index > 0 && <Button type="button" className="absolute bg-red-400 hover:bg-red-500 top-2 right-2 cursor-pointer" onClick={() => handleRemoveEmployee(index)} isActionLoading={isActionLoading}>
              Remove
            </Button>}
        </div>)}
      ...
      <div className="flex items-center gap-4 mt-6">
        ...
        <Button className="cursor-pointer" type="button" size="lg" onClick={handleSubmit} disabled={loading} isActionLoading={isActionLoading}>
